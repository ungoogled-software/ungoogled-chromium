...
* If the build fails with a patch-related message, it may be due to the fact that patches haven't been updated yet for macOS. Those types of errors are expected. You can see the status of patches in the validate_patches CI task in commits, [like this one](https://github.com/Eloston/ungoogled-chromium/runs/61780678).
